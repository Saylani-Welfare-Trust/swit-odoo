...
# Add these fields to existing welfare.line model
class WelfareLine(models.Model):
    _inherit = 'welfare.line'
    
    # Donee Information Fields (related from welfare_id)
    donee_name = fields.Char(
        string="Donee Name",
        related='welfare_id.name',
        store=True,
        readonly=True
    )
    
    donee_address = fields.Text(
        string="Donee Address",
        compute='_compute_donee_address',
        store=True
    )
    
    donee_mobile = fields.Char(
        string="Mobile",
        related='welfare_id.mobile',
        store=True,
        readonly=True
    )
    
    donee_cnic = fields.Char(
        string="CNIC",
        related='welfare_id.cnic_no',
        store=True,
        readonly=True
    )
    
    @api.depends('welfare_id')
    def _compute_donee_address(self):
        """
        Compute full address from welfare/donee record
        """
        for rec in self:
            if rec.welfare_id:
                address_parts = []
                if rec.welfare_id.street:
                    address_parts.append(rec.welfare_id.street)
                if rec.welfare_id.street2:
                    address_parts.append(rec.welfare_id.street2)
                if rec.welfare_id.city:
                    address_parts.append(rec.welfare_id.city)
                if rec.welfare_id.state_id:
                    address_parts.append(rec.welfare_id.state_id.name)
                if rec.welfare_id.zip:
                    address_parts.append(rec.welfare_id.zip)
                
                rec.donee_address = ', '.join(address_parts) if address_parts else False
            else:
                rec.donee_address = False