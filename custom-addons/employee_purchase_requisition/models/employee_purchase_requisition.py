...
class PurchaseRequisition(models.Model):
    ...
    user_id = fields.Many2one(
        ...
        help='Select a user who is responsible for the requisition'
    )
    ...
    @api.model
    def _get_default_delivery_type(self):
        picking_type = self.env['stock.picking.type'].search(
            [('name', 'ilike', 'epr')],
            limit=1
        )
        return picking_type.id or False

    delivery_type_id = fields.Many2one(
        comodel_name='stock.picking.type',
        string='Delivery To',
        help='Type of delivery.',
        default=_get_default_delivery_type,
    )
