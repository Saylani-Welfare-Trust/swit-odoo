...
class Chequepopup extends AbstractAwaitablePopup {
    setup() {
        ...
        this.get_report_category_analysis = async () => {
            ...
            try {
                const result = await this.ormService.call('pos.order', 'get_cheque_order', [this.state.activeId,offset, this.state.limit]);
    ...
    async searchTasks(){
        ...
        try {
            const result = await this.ormService.call('pos.order', 'get_cheque_order_specific', [this.state.activeId,text]);
            this.state.chequeorder = result.orders;
            this.state.totalRecords = result.total_count;
