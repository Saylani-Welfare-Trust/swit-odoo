...
class PosOrder(models.Model):
    ...
    def get_cheque_order(self, offset=0, limit=10):
            
        order = self.env['pos.order'].search([('cheque_order', '=', True), ('cheque_status', '!=', 'clear')], offset=offset, limit=limit)
        total_count = self.env['pos.order'].search_count([('cheque_order', '=', True),('cheque_status', '!=', 'clear')])  # Get total count of records
        data = []
        for i in order:
            status = ""
    ...
    def get_cheque_order_specific(self,text):
        if text:
            order = self.env['pos.order'].search([('cheque_order', '=', True),('cheuque_number','=',text)])
            total_count = self.env['pos.order'].search_count([('cheque_order', '=', True)])  # Get total count of records
            data = []
            for i in order:
                temp = {
                    "id":i.id,
