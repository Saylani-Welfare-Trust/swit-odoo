...
class EhBoardItem(models.Model):
    ...
    def _sanitize_visual_vals(self, vals):
        ...
        if "chart_options" in vals:
            ...
            colors = options.get("series_colors") if isinstance(options, dict) else []
    ...
    def _meta(self):
        ...
        return {
            ...
            "dimension_relation": self.primary_dimension_id.relation
            if self.primary_dimension_id else None,
    ...
    def _resolve_record_list_spec(self, options=None):
            ...
            for name in (self.list_field_order or [])[:12]:
                ...
                if column:
                    fields_spec.append({
                        ...
                        "type": {
                            "number": "float", "date": "date", "boolean": "boolean",
        ...
        order = "id desc"
        if (self.sort_field_id and self.sort_field_id.model_id == ds.model_id
                and self.sort_field_id.store
                and self.sort_field_id.ttype not in ("one2many", "many2many", "binary")):
            order = "%s %s, id %s" % (
                self.sort_field_id.name,
    ...
    def _ordered_list_fields(self):
        ...
        self.ensure_one()
        by_name = {field.name: field for field in self.list_field_ids}
        ordered_ids = []
        seen = set()
        for name in self.list_field_order or []:
            if name in by_name and name not in seen:
                ...
                seen.add(name)
        for field in self.list_field_ids:
            if field.name not in seen:
                ordered_ids.append(field.id)
                seen.add(field.name)
        return self.env["ir.model.fields"].browse(ordered_ids)
