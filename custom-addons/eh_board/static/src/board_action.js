...
export class BoardAction extends Component {
    ...
    get options() {
        ...
        for (const c of (this.state.crossFilters || [])) {
            const key = [c.model || "", c.relation || "", c.field].join("|");
            const entry = byField[key] || {
                field: c.field, model: c.model, relation: c.relation, values: new Set(),
            };
    ...
    toggleCrossFilter(field, value, label, model = null, relation = null) {
        const key = [model || "", relation || "", field, String(value)].join(":");
        const i = this.state.crossFilters.findIndex((c) => c.key === key);
        if (i >= 0) {
            this.state.crossFilters.splice(i, 1);
        } else {
            this.state.crossFilters = [
                ...this.state.crossFilters, { key, field, value, label, model, relation }];
        }
        this.refreshItems();
    }
            ...
            if (row && row.keys && row.keys.length) {
                ...
                if (key !== null && key !== undefined) {
                    this.toggleCrossFilter(meta.dimension, key,
                        ...
                        meta.model, meta.dimension_relation);
                    return;
                }
            }
        }
