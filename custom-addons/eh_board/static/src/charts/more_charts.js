...
export class FunnelChart extends Component {
    ...
    onHover(ev, seg) {
        showTooltip(ev, seg.label, [{ label: "Value", value: formatFor(this, seg.value), color: seg.color }]);
    }
    onMove(ev) { moveTooltip(ev); }
    onLeave() { hideTooltip(); }
}
...
export class ScatterChart extends Component {
    ...
    get xLabel() {
        const series = this.props.payload.series || [];
        return series[1] ? series[1].label : "Category index";
    }
    get yLabel() {
        const series = this.props.payload.series || [];
        return series[0] ? series[0].label : "Value";
    }
...
export class PolarChart extends Component {
    ...
    onHover(ev, s) { showTooltip(ev, s.label, [{ label: "Value", value: formatFor(this, s.value), color: s.color }]); }
    onMove(ev) { moveTooltip(ev); }
    onLeave() { hideTooltip(); }
}
...
export class RadialChart extends Component {
    ...
    onHover(ev, r) { showTooltip(ev, r.label, [{ label: "Value", value: formatFor(this, r.value), color: r.color }]); }
    onMove(ev) { moveTooltip(ev); }
    onLeave() { hideTooltip(); }
}
