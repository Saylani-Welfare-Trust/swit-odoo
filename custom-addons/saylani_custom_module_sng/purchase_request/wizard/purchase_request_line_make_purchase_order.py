...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        supplier_ids = request_lines.mapped("supplier_id")
        print('find', supplier_ids)
        # if len(supplier_ids) == 1:
        res["supplier_id"] = supplier_ids
        return res
    ...
    def _get_purchase_line_name(self, order, line):
        ...
        product_lang = line.product_id.with_context(
            ...
            partner_id=self.supplier_id.ids,
    ...
    def make_purchase_order22(self):
        res = []
    ...
    def make_purchase_order(self):
        print("=== Starting make_purchase_order ===")
        if not self.item_ids:
            print("No items in wizard.")
            return
        ...
        for supplier in self.supplier_id:
            ...
            for item in self.item_ids:
                if not item.product_id:
                    print(f"Skipping item {item.id}: missing product")
                    continue
                ...
                origin = item.request_id.origin


            if not order_line_values:
                print(f"No valid order lines for supplier {supplier.name}")
                continue
            ...
            print(f"Created Purchase Order: {po.name} for Supplier: {supplier.name}")
        return True
        ...
        if self.product_id:
            ...
            sup_info_id = self.env["product.supplierinfo"].search(
                [
                    ...
                    ("partner_id", "=", self.wiz_id.supplier_id.ids),
                ]
            )
            if sup_info_id:
