...
class APIDonationWizard(models.TransientModel):
    ...
    def _accumulate_donation_lines_fast(self, donation_vals, all_data, company_currency,
        ...
        for it in donation_vals.get('donation_item_ids', []):
            ...
            # Convert to company currency (multiply by rate)
            ...
            if is_foreign:
                ...
                foreign_amount = item_total_base  # same currency, no difference

            # Debit accumulation (debit_base in company currency, amount_currency positive)
            ...
            if is_foreign:
                d['amount_currency'] += foreign_amount

            # Credit accumulation (credit_base in company currency, amount_currency positive)
            ...
            if is_foreign:
                c['amount_currency'] += foreign_amount
    ...
    def _create_grouped_journal_move(self, journal, debit_accumulator, credit_accumulator, company_currency, history):
        ...
        for (account_id, currency_id), vals in debit_accumulator.items():
            ...
            if currency_id == company_currency_id:
                ...
                # Foreign currency: amount_currency must be positive
                lines.append((0, 0, {
                    'account_id': account_id,
                    'debit': debit_amount,
                    'credit': 0.0,
                    'currency_id': currency_id,
                    'amount_currency': abs(vals['amount_currency']),  # ensure positive
                    'name': 'Donation Import - Debit',
                }))
