...
class KeyIssuance(models.Model):
    ...
    box_no = fields.Char(related='donation_box_id.box_no', string='Box No', store=True, tracking=True)
    ...
    def set_donation_amount(self, data):
        ...
        if int(data['amount']) > 0:
            try:
                journal_entry_id = key_obj._create_donation_journal_entry(int(data['amount']))
                return {
                    "status": "success",
